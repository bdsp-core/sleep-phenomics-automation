...
from tensorflow.keras.layers import Input, BatchNormalization, Cropping2D, \
                                    ...
                                    UpSampling2D, ZeroPadding2D, Lambda, Conv2D, Dropout, \
...
class USleep_aux_psd(Model):
    ...
    def create_dense_modeling_PSD(self,
                              in_1,
                              in_2,
                              in_reshaped,
                              filters,
    ...
    @staticmethod
    def create_seq_modeling_PSD(in_1,
                            in_2,
                            input_dims,
                            data_per_period,
                            n_periods,
                            ...
                            **other_conv_params):
        cls = AveragePooling2D((data_per_period, 1),
                               name="{}average_pool".format(name_prefix))(in_1)
        merge = Concatenate()([cls,in_2])
        ...
        out = Conv2D(filters= 128,
                     ...
                     bias_regularizer=regularizer,
                     padding="same",
                     name="{}sequence_conv_out_3".format(name_prefix),
                     **other_conv_params)(out)
        out = Conv2D(filters= 64,
                     kernel_size=(transition_window, 1),
                     activation=activation,
                     ...
                     bias_regularizer=regularizer,
                     padding="same",
                     name="{}sequence_conv_out_4".format(name_prefix),
                     **other_conv_params)(out)
        out = Conv2D(filters=n_classes,
                     kernel_size=(transition_window, 1),
                     activation="softmax",
                     ...
                     bias_regularizer=regularizer,
                     padding="same",
                     name="{}sequence_conv_out_5".format(name_prefix),
                     **other_conv_params)(out)
        s = [-1, n_periods*input_dims//data_per_period, n_classes] ## output shape you can change this
        if len(s)>2:
            if s[2] == 1:
                s.pop(2)  # Squeeze the dim
        out = Lambda(lambda x: tf.reshape(x, s),name="{}".format(name_prefix))(out)
    ...
    def init_model(self, inputs=None, name_prefix=""):
        ...
        inputs = Input(shape=[self.n_periods,
                                self.input_dims,
                                self.n_channels],name='Input_EEG')
        inputs2 = Input(shape=[2100,1,4],name='Input_PSD')
        reshaped = [-1, self.n_periods*self.input_dims, 1, self.n_channels]
        in_reshaped = Lambda(lambda x: tf.reshape(x, reshaped))(inputs)
        ...
        Sequence modeling branch 2
        """
        out_arousal = self.create_seq_modeling_PSD(in_1=cls2,
                                       in_2=inputs2,
                                       input_dims=self.input_dims,
                                       data_per_period=self.data_per_prediction_b2,
                                       n_periods=self.n_periods,
        ...
        return [inputs,inputs2], [out_hypno,out_arousal]
